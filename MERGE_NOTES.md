...
### Sync `twenty/v2.16.1` — overridable-entity plumbing × ROADMAP view

Upstream v2.16.1 introduced the overridable-entity plumbing on the `view`
aggregate (`overrides` / `universalOverrides` / `isCallerOverridingEntity` /
`sanitizeOverridableEntityInput` / `isActive` / `isSystemSideEffect` /
`kanbanColumnWidth`) plus several `type`-import → value-import changes and a
relaxed Kanban main-group-by rule (SELECT **or** MANY_TO_ONE relation). The
fork never had any of that, so every flat-view file conflicted with our
ROADMAP additions. Resolution rule applied everywhere: **keep theirs for the
overridable plumbing and import-kind changes; re-add the fork's ROADMAP
surface re-expressed against theirs' renamed variables.**

| #   | Path                                                                                                                                                                          | Reason for divergence                                                                                                                                                                                                                                                  | Rebase strategy                                                                                                                                  |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| 96  | `packages/twenty-server/src/engine/metadata-modules/flat-view/utils/from-update-view-input-to-flat-view-to-update-or-throw.util.ts`                                          | Took theirs' `sanitizeOverridableEntityInput`/`mergedRecord`/`overrides`/`universalOverrides` flow; re-added the 9 ROADMAP FK re-resolve `if`-blocks re-keyed onto `updatedEditableProperties.<x>` (condition) + `mergedRecord.<x>` (FK value).                          | On rebase, keep theirs' merge flow; re-apply the 9 roadmap `if`-blocks with the then-current merged-record variable names. Never the old `updatedEditableFieldProperties`/`flatViewToUpdate` reads. |
| 97  | `packages/twenty-server/src/engine/metadata-modules/view/dtos/inputs/update-view.input.ts`                                                                                  | Kept theirs' `kanbanColumnWidth` `@IsInt/@Min/@Max` field; re-added the fork's 9 `roadmapField*Id` UUID inputs + scalar toggles (`roadmapDefaultZoom`, `roadmapShowToday`, `roadmapShowWeekends`, `roadmapShowDeviation`) ahead of it.                                    | Roadmap inputs and `kanbanColumnWidth` are additive siblings; keep both, each with its own `@IsOptional()`.                                       |
| 98  | `packages/twenty-server/src/engine/workspace-manager/workspace-migration/workspace-migration-builder/validators/services/flat-view-validator.service.ts`                    | Conflict was imports-only (body already merged). Merged to: value `RelationType`+`ViewType`+`FieldMetadataType`, kept `isFieldMetadataDateKind` (fork) and `isMorphOrRelationUniversalFlatFieldMetadata` (theirs), single `UniversalFlatView` import (no dup).            | Keep theirs' `isAllowedKanbanMainGroupByField` helper and the relaxed Kanban rule; keep the fork's `collectRoadmapValidationErrors` + its 2 call-sites. Dedupe `UniversalFlatView`. |
| 99  | `packages/twenty-server/src/engine/workspace-manager/workspace-migration/workspace-migration-runner/action-handlers/view/services/create-view-action-handler.service.ts`    | Kept theirs' `overrides` block + `isDefined` import; re-added the 9 resolved ROADMAP FK ids both in the `resolveUniversalRelationIdentifiersToIds` destructure and the `flatEntity` payload (both already present pre-merge; only the payload conflicted).               | Keep the 9 roadmap FK assignments and `overrides` together in the `flatEntity` literal.                                                          |
| 100 | `packages/twenty-server/src/engine/core-modules/application/application-manifest/converters/from-view-manifest-to-universal-flat-view.util.ts`                               | Manual object literal. Took theirs' `shouldHideEmptyGroups: viewManifest.shouldHideEmptyGroups ?? false` and `kanbanColumnWidth ?? null`; re-added the fork's static ROADMAP defaults (9 `roadmapField*UniversalIdentifier: null` + 4 scalar defaults).                  | `UniversalFlatView` requires every roadmap prop — compile is the test. Keep theirs' scalar exprs.                                                 |
| 101 | `packages/twenty-server/src/engine/workspace-manager/twenty-standard-application/utils/view/create-standard-view-flat-metadata.util.ts`                                      | Conflict was imports-only (body already merged). Made `ViewType`+`ViewKey` value imports (used as `ViewType.FIELDS_WIDGET`/`ViewKey.INDEX`), removed the now-duplicate `type ViewType`. Body already had theirs' `kanbanColumnWidth`/`isActive`/`overrides`/`isSystemSideEffect`. | Keep `ViewType`/`ViewKey` as value imports; never re-introduce a `type ViewType` line.                                                            |
| 102 | `packages/twenty-server/src/engine/twenty-orm/entity-manager/workspace-entity-manager.spec.ts`                                                                              | Feature-flag mock: kept theirs' 4 new flags + re-added `IS_ROADMAP_VIEW_ENABLED: false`. Also **re-authored** the fork's malformed roadmap aggregator mock lines (`roadmap{Label,PlannedStart,PlannedEnd,Status,BlockedBy}View{Ids,UniversalIdentifiers}`) one-per-line. | When the upstream fixture churns, re-add the roadmap aggregator defaults one property per line — do not copy the previously-malformed concatenated lines. |

#### Post-merge fixes (surfaced by typecheck / lint, not by conflict markers)

These files auto-merged with **no conflict markers** but broke against v2.16.1 type/tooling changes. Watch for them on the next sync:

- **`isUIReadOnly` → `isUIEditable` (inverted)** — upstream renamed the field-context property (instance command `2-13-…-rename-is-ui-read-only-to-is-ui-editable`). Fixed in 7 `compute-*-standard-flat-field-metadata.util.ts` (OpportunityMilestone, OpportunityMilestoneDependency + the extended attachment/opportunity/task-target/timeline-activity/workspace-member relations): `isUIReadOnly: true` → `isUIEditable: false`.
- **`search-fields-by-standard-object-name.constant.ts`** — upstream made it a `Record<AllStandardObjectName, …>`; the fork's 2 new standard objects became required keys. Added `opportunityMilestone: [{ name: 'name', TEXT }]` and `opportunityMilestoneDependency: [{ name: 'id', UUID }]`.
- **`2-10 sync-call-recording-standard-objects.command.ts`** — upstream raw `FlatFieldMetadata<SELECT>` literal lacked the fork's 9 roadmap aggregator arrays. Added `roadmap{Start,End,Group,Color,Label,PlannedStart,PlannedEnd,Status,BlockedBy}ViewIds: []` + matching `…ViewUniversalIdentifiers: []`. **Any new upstream code that builds a complete `FlatFieldMetadata` literal needs these 18 props** (the fork extended the type).
- **Frontend `twenty-ui/display` → `twenty-ui/icon`** — upstream split the package into `data-display` + `icon` entrypoints. Fixed icon imports in `ViewType.ts`, `SidePanelPageLayoutRecordPageWidgetTypeSelect.tsx`, `MilestonesCard.tsx`, `RecordRoadmapBar.tsx`, `RecordRoadmapDependencyPopover.tsx`, `ObjectOptionsDropdownRoadmapFieldPickerContent.tsx`. **All fork `twenty-ui/display` imports must move to `twenty-ui/icon` (icons) or `twenty-ui/data-display`.**
- **`ViewType.ts` `VIEW_TYPE_LABELS`** — added `[ViewType.ROADMAP]: msg\`Roadmap\`` (the `satisfies Record<ViewTypeLabelKey, …>` became non-exhaustive once the generated enum gained `ROADMAP`). `ViewPickerTypeSelectOptions.ts` now reads `VIEW_TYPE_LABELS[ViewType.ROADMAP]` instead of a bare `msg`.
- **`useDeleteOneObjectMetadataItem.ts`** — dropped the fork's `replaceDraft('commandMenuItems', …)` block (upstream removed `replaceDraft`; its `useInvalidateMetadataStore` superset already covers the fork's original intent) and removed a duplicate `useInvalidateMetadataStore` import.
- **Version constants** — took theirs verbatim (`TWENTY_CURRENT_VERSION = '2.17.0'`, previous list up to `2.16.0`) to keep the upgrade subsystem internally consistent. If the admin-panel inferred version must read `2.16`, apply a revert like commit `f1125d0894` — the cosmetic constant only, never the instance-command sequence.
- **Instance-command filenames** — renamed the 2 fork files to kebab-case (`add-milestones-to-page-layout-widget-type-enum`, `add-markdown-to-page-layout-widget-type-enum`) to satisfy the `twenty(upgrade-command-filename)` oxlint rule; updated imports in `instance-commands.constant.ts`. Also dropped a duplicate `AddRelationTargetFieldMetadataIdToViewFilterFastInstanceCommand` array registration that the merge introduced.
- **`generated-metadata/graphql.ts` + `generated/graphql.ts`** — taken from upstream during the merge, then **regenerated** against the running fork server (`graphql:generate` + `--configuration=metadata`). Never hand-merge these.

#### Sync verification (v2.16.1) — what was run

- Environment: upstream now requires **Node `^24.5.0`** (`.nvmrc` 24.16.0). Node 22 fails `yarn install`. Verified on local nvm `v24.14.1`.
- `twenty-shared` build ✅ · `twenty-server` typecheck ✅ (0) · `twenty-front` typecheck ✅ (0) · oxlint + oxfmt both packages ✅ (1 pre-existing upstream unused-import **warning** in `update-subscription-quantity.job.ts`).
...
### Roadmap — Notion-parity update (frontend-only)

Closes the gap with Notion's Timeline view. **Frontend-only** — no schema, GraphQL, or migration changes, so no upstream-merge conflict surface beyond the `record-roadmap/` module itself. Branch `feature/roadmap-notion-parity-spv`.

Also **documents a feature that was previously absent from this log**: vertical drag-and-drop **reorder-by-position** (commits `ee7caaa4c0` / `349cac249d`). The bar's `move` drop onto another row of the same swimlane writes `position` via `computeNewPositionOfDraggedRecord` → `useRecordRoadmapUpdateDates`. This update makes that gesture discoverable (grip handle) and adds the sort-conflict guard it always needed.

| Area | Files | Change |
| --- | --- | --- |
| **View sort + filter honored** | `record-roadmap/hooks/useRecordRoadmapFetchRecords.ts`, `useRecordRoadmapSwimlanes.ts` | Fetch now computes `orderBy` via `turnSortsIntoOrderBy(objectMetadataItem, currentRecordSorts, objectMetadataItems)` and `filter` via `computeRecordGqlOperationFilter`/`combineFilters` — read from the shared record-index instance context (no instanceId needed). Swimlane hook receives `hasActiveSort` and preserves server order (stable `() => 0` compare) instead of re-sorting by `position` when a sort is active. |
| **Relation labels (not UUIDs)** | `useRecordRoadmapFetchRecords.ts`, `RecordRoadmapTimeline.tsx`, `RecordRoadmapNameColumn.tsx`, new `utils/resolveRoadmapRelationLabel.ts` | Fetch hook resolves RELATION fields via `generateDepthRecordGqlFieldsFromFields` (real `labelIdentifier` per target) instead of a hardcoded `{ id, name }`. New util resolves the related record's labelIdentifier (handles FULL_NAME); Timeline uses it for the bar label + a precomputed `recordId→fieldId` map passed to the name-column RELATION cells. Unit-tested (`utils/__tests__/resolveRoadmapRelationLabel.test.ts`). |
| **Visible bar dates** | `RecordRoadmapBar.tsx` | `StyledBarDateLabel` renders the start–end range (muted, Notion style) as an absolutely-positioned sibling to the RIGHT of the bar — never inside the `overflow:hidden` box, so connector/port geometry is untouched. Gated to `dayWidthPx >= 24` (Day/Week zoom). |
| **Discoverable reorder + sort guard** | `RecordRoadmapNameColumn.tsx`, new `hooks/useRecordRoadmapRowReorderInteraction.ts`, new `utils/recordRoadmapRowDragDom.ts`, `RecordRoadmapTimeline.tsx`, `useRecordRoadmapBarInteraction.ts` | Grip handle (`IconGripVertical`, hover-revealed) per name-column row drives the existing position-write path through a lightweight pointer hook. Shared DOM helpers (`findRoadmapRowRecordIdAtPoint` / `updateRoadmapDropTargetHighlight`) extracted from the bar hook so both gestures stay identical. Timeline gates reorder on `hasActiveSort` → opens `RECORD_INDEX_REMOVE_SORTING_MODAL_ID` (now mounted in the roadmap, was Table/Board-only). |
| **Design polish** | `RecordRoadmapBar.tsx`, `RecordRoadmapTopBar.tsx`, `RecordRoadmapTimeline.tsx`, `RecordRoadmapNameColumn.tsx`, `RecordRoadmapTimeHeader.tsx`, `RecordRoadmapTodayLine.tsx`, new `states/recordRoadmapNameColumnWidthComponentState.ts` | Rounded bars (6px) + hover box-shadow; on-bar status pill (`StyledBarChip`, Day/Week zoom, wide bars); resizable name column (drag-divider → component-state atom); MONTH zoom exposed in the TopBar switch (was the default but unselectable); MONTH zoom's empty header lower-tier filled with weeks; "Today" pill repositioned below the sticky header (was hidden at `top:-18px`). |

**Sync guidance:** all changes live under `packages/twenty-front/src/modules/object-record/record-roadmap/` plus reuse of upstream utilities (`turnSortsIntoOrderBy`, `generateDepthRecordGqlFieldsFromFields`, `generateDefaultRecordChipData`, `computeNewPositionOfDraggedRecord`, `RECORD_INDEX_REMOVE_SORTING_MODAL_ID`). If upstream renames any of those, re-point the imports. No `RoadmapPlacedRecord`/view-entity/DTO shape changed.

**Validation:** `twenty-front` typecheck ✅ (0) · oxlint+oxfmt diff-with-main ✅ (0/0) · jest `resolveRoadmapRelationLabel` ✅ (6/6). Interaction/visual flows (live sort reorder, grip drag, on-bar dates/chips, column resize) to be confirmed in-browser against the milestone Roadmap view.

#### Follow-ups (same branch, all in `record-roadmap/`)

- **Vertical scroll over the name column** — the left pane is `overflow-y:hidden` and only mirrors the canvas `scrollTop`, so wheeling over the labels did nothing. Added an `onWheel` on `StyledNameColumnScroller` that forwards `deltaY`/`deltaX` to the canvas (which mirrors back), plus a second `useRecordRoadmapWheelZoom(nameColumnScrollerRef)` so Ctrl/Cmd+wheel zooms there instead of browser-zooming.
- **Relation columns show the record image** — the name-column RELATION cell now renders `RecordChip` (avatar/logo + label, `ChipVariant.Transparent`, `forceDisableClick`) instead of plain text, so a Company shows its logo, a Person their avatar, etc. Works because the fetch already requests the target's image identifier (via `generateDepthRecordGqlFieldsFromFields`); `useRecordChipData`'s `PreComputedChipGeneratorsContext` is provided app-wide in `AppRouterProviders`. Falls back to the resolved label text.
- **Zoom persists to the view** — the live zoom (`recordRoadmapZoomComponentState`) is now seeded from `view.roadmapDefaultZoom` on load and persisted back on change via `useUpdateCurrentView({ roadmapDefaultZoom })` (debounced 600ms; the column already existed but was never wired). Persists only on editable views.
