...
// SPV: roadmap view configuration has no manifest slot — the inverse
// converter resets it to defaults, so an exported roadmap view comes back
// as an unconfigured one.
const ROADMAP_VIEW_GAPS = Object.fromEntries(
  [
    'roadmapDefaultZoom',
    'roadmapShowToday',
    'roadmapShowWeekends',
  ...
  ].map((property) => [property, 'no manifest slot for roadmap configuration']),
);
...
const EXPORTED_KINDS: ExportedKind[] = [
  ...
  },
  {
    ...
    knownGaps: { ...VIEW_KIND_GAPS, ...ROADMAP_VIEW_GAPS },
  },
  {
